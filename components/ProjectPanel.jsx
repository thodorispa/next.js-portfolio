...
export default function ProjectPanel() {
    ...
    if (!isEqual(originalProject, editedProject)) {
      try {
        if (editedProject?.id === -1) {
          const { data } = await axios.post('/api/project/upload', editedProject)
  ...
  return (
    <div className={`panel ${panel ? '' : 'hidden'}`}>
      <header>
        <h1>{project._id ? "Edit project" : "Create new project"}</h1>
          ...
          {editedProject?._id ? (
            <button
              ...
              onClick={() => {
                ...
                if (!c) return
                axios.post('/api/project/delete', { id: editedProject?._id })
